...
class _DeveloperSettingsPageState extends State<DeveloperSettingsPage> {
  ...
  Future<void> _saveCenterApiUrl() async {
    ...
    if (normalized.isEmpty) {
      AppSnackbar.showSnackbarError('Please enter backend URL');
      return;
    }
    Uri? parsed;
    try {
    ...
    if (parsed == null || !(parsed.isScheme('http') || parsed.isScheme('https')) || parsed.host.isEmpty) {
      AppSnackbar.showSnackbarError('Invalid URL. Use http(s)://host[:port]/');
      return;
    }

    SharedPreferencesUtil().lifetraceApiBaseUrl = normalized;
    ...
    AppSnackbar.showSnackbarSuccess('Center node URL updated');
  }

  Future<void> _resetCenterApiUrl() async {
    final defaultUrl = const LifeTraceEnv().apiBaseUrl ?? '';
    SharedPreferencesUtil().lifetraceApiBaseUrl = '';
    ...
    AppSnackbar.showSnackbarSuccess('Center node URL reset to default');
  }

  Future<void> _pingCenterApi() async {
    final normalized = _normalizeApiBaseUrl(_centerApiUrlController.text);
    if (normalized.isEmpty) {
      AppSnackbar.showSnackbarError('Please enter backend URL first');
      return;
    }
    ...
    setState(() {
      ...
      if (response == null) {
        ...
        _centerApiPingResult = 'Ping failed: no response (${elapsed}ms)';
        return;
      }
      final ok = response.statusCode == 200;
      _centerApiPingOk = ok;
      _centerApiPingResult = ok
          ? 'Ping success: HTTP 200 (${elapsed}ms)'
          : 'Ping failed: HTTP ${response.statusCode} (${elapsed}ms)';
    });
  }
