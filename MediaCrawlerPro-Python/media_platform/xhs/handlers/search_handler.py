        ...
        for keyword in keyword_list:
            ...
            while saved_note_count <= config.CRAWLER_MAX_NOTES_COUNT:
                try:
                    ...
                    note_id_list, xsec_tokens, new_notes_count = (
                    ...
                    page += 1
                    # 如果开启了跳过已存在帖子的功能，只计数新帖子
                    if config.SKIP_EXISTING_NOTES and config.SAVE_DATA_OPTION in ("db", "csv"):
                        saved_note_count += new_notes_count
                        utils.logger.info(
                            f"[SearchHandler.search] New notes in this batch: {new_notes_count}, total new notes: {saved_note_count}"
                        )
                    else:
