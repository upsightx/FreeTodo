        ...
        for keyword in keyword_list:
            ...
            while saved_note_count <= config.CRAWLER_MAX_NOTES_COUNT:
                try:
                    ...
                    if config.SKIP_EXISTING_NOTES and config.SAVE_DATA_OPTION in ("db", "csv"):
                        saved_note_count += new_videos_count
                        utils.logger.info(
                            f"[SearchHandler.search] New videos in this batch: {new_videos_count}, total new videos: {saved_note_count}"
                        )
                    else:
