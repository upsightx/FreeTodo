...
class TieBaExtractor:
    ...
    def extract_note_detail(self, page_content: str) -> TiebaNote:
        ...
        # 提取帖子内容中的图片（主楼的图片）
        image_list = self.extract_post_images(first_floor_selector)
    ...
    def extract_post_images(self, post_selector: Selector) -> str:
        """
        提取帖子内容中的图片URL列表
        Args:
            post_selector: 帖子内容的选择器
        ...
        # 贴吧图片可能在多种标签中：
        # 1. BDE_Image 类的 img 标签（常见的帖子图片）
        # 2. d_post_content 内的 img 标签
        # 3. 带有 class="BDE_Image" 的图片
        
        # 方式1：提取 BDE_Image 类的图片
        bde_images = post_selector.xpath(".//img[@class='BDE_Image']/@src").getall()
        images.extend(bde_images)
        
        # 方式2：提取帖子内容区域的图片
        content_images = post_selector.xpath(".//div[contains(@class, 'd_post_content')]//img/@src").getall()
        for img_url in content_images:
            if img_url not in images and self._is_valid_image_url(img_url):
                images.append(img_url)
        
        # 方式3：提取 cc 下的图片（贴吧特有的图片容器）
        cc_images = post_selector.xpath(".//cc//img/@src").getall()
        for img_url in cc_images:
            if img_url not in images and self._is_valid_image_url(img_url):
                images.append(img_url)
        ...
        for img_url in images:
            if self._is_valid_image_url(img_url):
                ...
                if img_url.startswith("//"):
                    ...
                    img_url = "https://tiebapic.baidu.com" + img_url
                valid_images.append(img_url)
        
        return ",".join(valid_images)
    
    def _is_valid_image_url(self, url: str) -> bool:
        """
        判断是否是有效的帖子图片URL（排除表情、头像、功能按钮等）
        """
        if not url:
            return False
        ...
        exclude_patterns = [
            ...
            "gss0.bdstatic.com/6LZ1dD3d1sgCo2Kml5_Y_D3/sys/portrait",  # 头像
            "himg.bdimg.com/sys/portrait",  # 头像
            "data:image",  # base64图片
            "loading",
            "jubao",  # 举报按钮
            "static-pb",  # 贴吧静态资源目录（包含功能按钮等）
            "tb2.bdstatic.com/tb/",  # 贴吧功能按钮等静态资源
            "bdstatic.com/tb/",  # 贴吧静态资源
            "button",  # 按钮图片
            "icon",  # 图标
        ]
        ...
        # 如果URL包含有效域名，则认为是有效图片
        for domain in valid_domains:
            if domain in url:
                return True
        
        # 如果是相对路径且看起来像图片，也认为有效
        if url.startswith("/") and (".jpg" in url or ".png" in url or ".gif" in url or ".jpeg" in url):
            return True
